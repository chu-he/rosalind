...
def fitting_align(a, b):
    ...
    for i in range(1, len(a)+1):
        ...
        for j in range(1, len(b)+1):
            ...
            match = (M[i-1, j-1] + (1 if a_ch == b_ch else -1), 1)
            a_gap = (M[i-1, j] - 1,                             2)
            b_gap = (M[i, j-1] - 1,                             3)
    ...
    # Move the j-pointer to the position at which all of B has been consumed
    ...
    # Backtrack until B has been completely consumed
    local_a = ''
    local_b = ''
    while True:
        a_ch = a[i-1]
...
if __name__=="__main__":
    ...
    best_score, local_a, local_b = fitting_align(a, b)
