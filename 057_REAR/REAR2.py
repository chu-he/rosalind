...
def find_reversal_distance(list, level=0):
    ...
    for i in range(num_breakpoints):
        for j in range(i+1, num_breakpoints):
            ...
            if swap_score >= best_swap_score:
                ...
                swap_map[swap_score].append(swapped)
    ...
    if False:
        #print(f'Swapping an increasing strip')
        increasing_swaps = swap_increasing_strip(list)
        #print(increasing_swaps)
        result = 1 + min([find_reversal_distance(p, level=level+1) for p in increasing_swaps])
        cache[cache_key] = result
        return result
        
    else:
        swap_possibilities = swap_map[best_swap_score]
        result = 1 + min([find_reversal_distance(p, level=level+1) for p in swap_possibilities])
        cache[cache_key] = result
        return result
...
if __name__=="__main__":
    ...
    for pair in pairs:
        ...
        print(pair)
        numReversals = find_reversal_distance(pair[1])
        print(numReversals)
